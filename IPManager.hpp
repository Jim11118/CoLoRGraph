...
public:
	...
	void setIps(int machines, string next) {
		...
		else if (starts_with(next, "EricMachines")) {
			vector<string> tok;
			split(tok, next, is_any_of(" "), token_compress_on);
			if (tok.size() != (machines+1)) {
				cout << "You have specified " << sizeof(tok)-1 << "  machines for Eric instead of " << machines << "\n";
			}
			for (int i = 1; i < tok.size(); i++) {
		...
		else if (starts_with(next, "FrankMachines")) {
			vector<string> tok;
			split(tok, next, is_any_of(" "), token_compress_on);
			if (tok.size() != (machines+1)) {
				cout << "You have specified " << sizeof(tok)-1 << "  machines for Frank instead of " << machines << "\n";
			}
			for (int i = 1; i < tok.size(); i++) {
