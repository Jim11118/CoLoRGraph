...
namespace Utility
{
	...
	std::vector<uint32_t> VectorOperation::Add(const std::vector<uint32_t>& x, const std::vector<uint32_t>& y)
	{
		std::vector<uint32_t> ret(x.size());
	...
	//add function
	std::vector<int32_t> VectorOperation::Add(const std::vector<int32_t>& x, const std::vector<int32_t>& y)
	{
		std::vector<int32_t> ret(x.size());
	...
	std::vector<uint32_t> VectorOperation::Sub(const std::vector<uint32_t>& x, const std::vector<uint32_t>& y)
	{
		std::vector<uint32_t> ret(x.size());
	...
	std::vector<uint32_t> VectorOperation::Mul(const uint32_t x, const std::vector<uint32_t>& y)
	{
		std::vector<uint32_t> ret(y.size());
	...
	std::vector<int32_t> VectorOperation::Mul(const int32_t x, const std::vector<int32_t>& y)
	{
		std::vector<int32_t> ret(y.size());
	...
	std::vector<uint32_t> VectorOperation::Mul(const std::vector<uint32_t>& x, const uint32_t y)
	{
		std::vector<uint32_t> ret(x.size());
	...
	std::vector<uint32_t> VectorOperation::Mul(const std::vector<uint32_t>& x, const std::vector<uint32_t>& y)
	{
		std::vector<uint32_t> ret(x.size());
	...
	std::vector<uint32_t> VectorOperation::Dot(const std::vector<uint32_t>& x, const std::vector<uint32_t>& y)
	{
	...
	std::vector<uint32_t> CryptoUtility::SampleSmallInput(int length){
		...
		{
			uint32_t rng = ((distribution(generator)) << 17);
			input.push_back(rng);
		}
	...
	std::vector<uint32_t> CryptoUtility::SampleUInt32Array(int length)
	{
...
// 		std::cout << "count: " << count << std::endl;
	...
	std::vector<uint32_t> ArrayEncoder::UCharVec2UInt32tVec(std::vector<unsigned char> array)
	{
		uint32_t *temp = (uint32_t *)array.data();
		std::vector<uint32_t> ret(temp, temp + array.size()/sizeof(uint32_t));
		return ret;
	}
	
	std::vector<unsigned char> ArrayEncoder::Encode(std::vector<uint32_t> array)
	{
		
 		// assert(array != nullptr);
		unsigned char *ptr = (unsigned char *)array.data();
		
		std::vector<unsigned char> newArray(ptr, ptr + array.size()*sizeof(uint32_t));

		return newArray;
	}
	
	std::vector<unsigned char> ArrayEncoder::Hash(std::vector<uint32_t> array)
	{
	...
	std::vector<uint32_t> ArrayEncoder::Decodeuint32_t(std::vector<unsigned char> &array)
	{
		uint32_t *temp = (uint32_t *)array.data();
		std::vector<uint32_t> ret(temp, temp + array.size()/sizeof(uint32_t));
		return ret;
	...
	std::vector<unsigned char> ArrayEncoder::EncodeUInt32Array(std::vector<std::vector<uint32_t> > array)
	{
		...
		uint32_t *data = new uint32_t[1 + array.size() + arraySize];
		...
		uint32_t *dataPtr = data + startPoint;
		...
		tempVec.insert(tempVec.end(), temp, temp + (1 + array.size() + arraySize)*sizeof(uint32_t));
		return tempVec;
	}
	
	std::vector<std::vector<uint32_t> > ArrayEncoder::DecodeUInt32Array(std::vector<unsigned char> array)
	{
 		// std::cout << "DecodeInt32Array: ";
		...
		std::vector<std::vector<uint32_t> > ret(0);
		
		for(int idx = 0; idx < count; idx++)
		{
			std::vector<uint32_t> temp(mulGatesPerLayer[idx]);
			uint32_t *dataPtr = data + startPoint;
	...
	std::vector<unsigned char> ArrayEncoder::MacVectorToByteArray(std::vector<uint32_t> MACedEdges)
	{
	...
	std::vector<unsigned char> ArrayEncoder::MacVectorToByteArray(uint32_t *MACedEdges, int size)
	{
		// 40 bit macs is equivalent to 5 bytes 
	...
	std::vector<uint32_t> ArrayEncoder::ByteArrayToMacVector(std::vector<unsigned char> byteArray)
	{
		assert(byteArray.size() % 5 == 0);
		
		std::vector<uint32_t> MACedEdges(byteArray.size()/5);
	...
	DistributedMACGenerator::DistributedMACGenerator(uint32_t alpha, uint32_t beta) : alpha(alpha), beta(beta)
	{
		uint32_t val = 1;
	...
	uint32_t DistributedMACGenerator::computeMAC(int party, std::vector<uint32_t> &data, uint32_t random)
	{
		uint32_t mac = 0;
...
// 		return mac;
// 		return (uint32_t) (mac % Z64);
		return (mac % p);
	}
	
	// SPDZ2kMAC::SPDZ2kMAC(uint32_t alpha, Communicator *com) : alpha(alpha), com(com){}
	
	uint32_t SPDZ2kMAC::computeMAC(int party, uint32_t data, uint32_t random)
	{
		if(party % 2 == 0)
		{
	...
	// uint32_t SPDZ2kMAC::singleCheck(int party, uint32_t mac, uint32_t share)
	// {
	// 	uint32_t ourShare = mac - alpha*share;
	// 	uint32_t theirShare;
		
	// 	if(party % 2 == 0){
	// 		com->SendEvaluationPartner((unsigned char *)(&ourShare), sizeof(uint32_t));
	// 	}
	// 	else
	// 	{
	// 		com->AwaitEvaluationPartner((unsigned char *)(&theirShare), sizeof(uint32_t));
	// 	}
	...
	void TestUtility::PrintVector(const std::vector<uint32_t>& input, const std::string& str){
		std::stringstream ss;
		ss << str << "\n";
		for(int idx = 0; idx < input.size(); idx++)
		{
	...
	void TestUtility::PrintVector(const std::vector<std::vector<uint32_t> >& input, const std::string& str, float scale){
		std::stringstream ss;
		ss << str << "\n";
		for(int idx = 0; idx < input.size(); idx++)
		{
	...
	void TestUtility::PrintMask(const std::vector<uint32_t>& masks, const std::vector<int>& maskIndex, const std::vector<int>& itemsPerUser, const std::string& str){
		// Count number of required random values: some wires need 10 masks, some 1
		int numUsers = itemsPerUser.size();
		int numEdges = 0;
		for(int idx = 0; idx < itemsPerUser.size(); idx++)
	...
	std::vector<std::vector<uint32_t> > TestUtility::GenerateInput(const std::vector<int>& itemsPerUser, int inputLength)
	{
		unsigned seed =  std::chrono::system_clock::now().time_since_epoch().count();
		std::default_random_engine generator(seed);
		...
		std::vector<std::vector<uint32_t> > input(inputLength);
		int count = 0;
