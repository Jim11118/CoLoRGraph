...
namespace TwoPartyMaskedEvaluation
{
	...
	public:
		...
		std::vector<std::vector<uint32_t> > beaverShare; // [lz + lx*ly]
	private:
	...
	public:
		...
		PreprocessingShare(std::vector<uint32_t>&& masks, std::vector<std::vector<uint32_t> >&& beaverSharesPerLayer);
		PreprocessingShare(std::vector<uint32_t>&& masks, std::vector<std::vector<uint32_t> >&& beaverSharesPerLayer, const std::vector<int>& maskIndex);
		PreprocessingShare(std::vector<uint32_t>&& masks, std::vector<std::vector<uint32_t> >&& beaverSharesPerLayer, std::vector<int>&& maskIndex);
		...
		uint32_t getMask(int index);
		
		std::vector<uint32_t>  operator[](Range *range);

		/// <summary>
		/// Returns the next beaver triple
		/// </summary>
		std::vector<uint32_t> GetNextBeaverTriples();
