...
// 		std::vector<uint32_t> masks(length);
		std::vector<uint32_t> masks = rng->GetMaskArray(length); //CryptoUtility::SampleUint32Array(length);
		
		std::vector<std::vector<uint32_t> > aliceBeaverShares;
		std::vector<std::vector<uint32_t> > bobBeaverShares;
		...
		for (int ilayer = 0; ilayer < lc->Depth; ilayer++)
		{
			...
			if (mulGates.size() > 0)
			{
				...
				std::vector<uint32_t> beaver(mulCount);
				...
				std::vector<uint32_t> aliceBeaverShare = CryptoUtility::SampleUInt32Array(mulCount);
				
				aliceBeaverShares.push_back(std::move(aliceBeaverShare));
				
				std::vector<uint32_t> bobBeaverShare(mulCount);
				for(int idx = 0; idx < mulCount; idx++)
				{
...
// 				delete [] beaver;
		...
		std::vector<uint32_t> masksShareAlice = CryptoUtility::SampleUInt32Array(length);
		
		std::vector<uint32_t> masksShareBob(length);
	...
	PreprocessingShareStorage *SimplePreprocessingBuilder::BuildPreprocessing(LayeredArithmeticCircuit *lc, std::vector<int>& itemsPerUser)
	{
		...
		std::vector<uint32_t> masks = rng->GetMaskArray(numMasks);
		t.Tick("AES time");
...
// 			std::cout << "Layer " << ilayer << std::endl;
	...
	// 			std::cout << "Mul Gates" << std::endl;
				...
				// For idx-th gate ( (x, y), z ): aliceBeaverShare[idx] = <lx*ly> and bobBeaverShares[idx] = <lx*ly>
				std::vector<uint32_t> aliceBeaverShare = CryptoUtility::SampleUInt32Array(DIM*mulCount);			
				std::vector<uint32_t> bobBeaverShare(DIM*mulCount);
	...
	// 			std::cout << "Dot Gates" << std::endl;
				...
				// For idx-th gate ( (x, y), z ): aliceBeaverShare[idx] = <lx*ly> and bobBeaverShares[idx] = <lx*ly>
				std::vector<uint32_t> aliceBeaverShare = CryptoUtility::SampleUInt32Array(dotCount);
				std::vector<uint32_t> bobBeaverShare(dotCount);
	...
	// 				std::cout << maskIndex[g->LeftWire] << " " << maskIndex[g->RightWire] << std::endl;
			...
			if (count != 0)
			{
				std::vector<uint32_t> aliceBeaverShare = CryptoUtility::SampleUInt32Array(DIM*count);
				std::vector<uint32_t> bobBeaverShare(DIM*count);
...
// 			t.Tick("Time for this layer");
		...
		/// secret share for wires
		// For wire x: masksShareAlice[x] = <lx> and masksShareBob[x] = <lx>
		std::vector<uint32_t> masksShareAlice = CryptoUtility::SampleUInt32Array(numMasks);
		t.Tick("Time to generate random share");
		std::vector<uint32_t> masksShareBob   = VectorOperation::Sub(masks, masksShareAlice);
