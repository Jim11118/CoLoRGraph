...
// 	void EmulatedMaskedEvaluation::AddInput(std::vector<uint32_t> input, Range *range)
// 	{
...
// 	}
	...
	void EmulatedMaskedEvaluation::AddInput(std::vector<std::vector<uint32_t> >&& input, Range *range)
	{
		maskedEvaluation = std::move(input);
		maskedEvaluation.resize(lc->NumWires);
...
// 	std::vector<uint32_t> EmulatedMaskedEvaluation::Decrypt(std::vector<uint32_t> mask, Range *range)
// 	{
...
// 	}
	...
	std::vector<std::vector<uint32_t> > EmulatedMaskedEvaluation::Decrypt(std::vector<uint32_t> mask, Range *range)
	{
// 		assert(mask->Length == range->Length);
...
// 		uint32_t *aliceBeaver = aliceShare->GetNextBeaverTriples();
// 		uint32_t *bobBeaver = bobShare->GetNextBeaverTriples();
	...
	void EmulatedMaskedEvaluation::EvaluateAddGate(int layer)
	{
		...
		{
			std::vector<uint32_t> leftValue  = maskedEvaluation[g->LeftWire];
			std::vector<uint32_t> rightValue = maskedEvaluation[g->RightWire];
...
// 			maskedEvaluation[outputWire] = ArithmeticOperation::sub(maskedEvaluation[leftWire], maskedEvaluation[rightWire]);
	...
	void EmulatedMaskedEvaluation::EvaluateCAddGate(int layer)
	{
		...
		{
			std::vector<uint32_t> inputValue = maskedEvaluation[g->InputWire];
...
// 			maskedEvaluation[outputWire] = ArithmeticOperation::add(maskedEvaluation[inputWire], g->constant);
	...
	void EmulatedMaskedEvaluation::EvaluateMulGate(int layer, PreprocessingShare *aliceBeaver, PreprocessingShare *bobBeaver)
	{
		...
		if (mulGates.size() > 0)
		{
			std::vector<uint32_t> aliceBeaverShare = aliceBeaver->GetNextBeaverTriples();
			std::vector<uint32_t> bobBeaverShare   = bobBeaver->GetNextBeaverTriples();
			...
			for (int i = 0; i < mulGates.size(); i++)
			{
				...
				uint32_t mx = maskedEvaluation[g->LeftWire][0];  // x + lx
				std::vector<uint32_t> my = maskedEvaluation[g->RightWire]; // y + ly

				int leftWire = g->LeftWire;
				int rightWire = g->RightWire;
				int outputWire = g->OutputWire;
				
				uint32_t lx1 = aliceShare->operator[]((*maskIndex)[leftWire]);
				uint32_t lx2 = bobShare->operator[]((*maskIndex)[leftWire]);
				
				for(int kdx = 0; kdx < DIM; kdx++)
				{
					uint32_t ly1 = aliceShare->operator[]((*maskIndex)[rightWire] + kdx);
					uint32_t lz1 = aliceShare->operator[]((*maskIndex)[outputWire] + kdx);
					uint32_t aliceBeaverValue = -ArithmeticOperation::mul(lx1, my[kdx]) - ArithmeticOperation::mul(ly1, mx) + lz1 + aliceBeaverShare[i*DIM + kdx];
					
					uint32_t ly2 = bobShare->operator[]((*maskIndex)[rightWire] + kdx);
					uint32_t lz2 = bobShare->operator[]((*maskIndex)[outputWire] + kdx);
					
					uint32_t bobBeaverValue = -ArithmeticOperation::mul(lx2, my[kdx]) - ArithmeticOperation::mul(ly2, mx) + lz2 + bobBeaverShare[i*DIM + kdx];
	...
	void EmulatedMaskedEvaluation::EvaluateDotGate(int layer, PreprocessingShare *aliceBeaver, PreprocessingShare *bobBeaver){
		...
		if(dotGates.size() > 0)
		{
			...
			for (int i = 0; i < dotGates.size(); i++)
			{
				...
				std::vector<uint32_t> mx = maskedEvaluation[g->LeftWire];  // x + lx
				std::vector<uint32_t> my = maskedEvaluation[g->RightWire]; // y + ly
				...
				for(int kdx = 0; kdx < DIM; kdx++)
				{
					uint32_t lx1 = aliceShare->operator[]((*maskIndex)[leftWire] + kdx);
					uint32_t ly1 = aliceShare->operator[]((*maskIndex)[rightWire] + kdx);
					
					uint32_t aliceBeaverValue = -ArithmeticOperation::mul(lx1, my[kdx]) - ArithmeticOperation::mul(ly1, mx[kdx]);
					
					uint32_t lx2 = bobShare->operator[]((*maskIndex)[leftWire] + kdx);
					uint32_t ly2 = bobShare->operator[]((*maskIndex)[rightWire] + kdx);
					
					uint32_t bobBeaverValue = -ArithmeticOperation::mul(lx2, my[kdx]) - ArithmeticOperation::mul(ly2, mx[kdx]);
					
					maskedEvaluation[g->OutputWire][0] += ArithmeticOperation::mul(mx[kdx], my[kdx]) + aliceBeaverValue + bobBeaverValue;
				}
				
				uint32_t lz1 = aliceShare->operator[]((*maskIndex)[outputWire]);
				uint32_t lz2 = bobShare->operator[]((*maskIndex)[outputWire]);
				maskedEvaluation[g->OutputWire][0] =  maskedEvaluation[g->OutputWire][0] + aliceBeaverShare[i] + bobBeaverShare[i] + lz1 + lz2;
			}
		}
	}

	uint32_t EmulatedMaskedEvaluation::BeaverEvaluation(MultiplicationGate *g, uint32_t beaver, uint32_t mx, uint32_t my, PreprocessingShare *share)
	{
		int leftWire = g->LeftWire;
		int rightWire = g->RightWire;
		int outputWire = g->OutputWire;

		uint32_t lx = share->operator[](leftWire);
		uint32_t ly = share->operator[](rightWire);
		uint32_t lz = share->operator[](outputWire);
