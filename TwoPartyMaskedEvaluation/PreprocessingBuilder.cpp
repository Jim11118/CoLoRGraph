...
namespace TwoPartyMaskedEvaluation
{
	...
	PreprocessingShare * MaskShareBuilder::Build(LayeredArithmeticCircuit *lc, AESRNG *rng, const std::vector<int>& itemsPerUser, const std::vector<int>& maskIndex)
	{
		...
		std::vector<uint32_t> maskShare = rng->GetUInt32Array(numInputMasks);
		...
		std::vector<uint32_t> beaverMask = rng->GetUInt32Array(nBeaverMasks);
		...
		std::vector<std::vector<uint32_t> > beaverShare;
...
// 		t.Tick("AlicePreprocessingBuilder::Build");
	...
	const std::vector<uint32_t>& PreprocessingBuilder::getMasks()
	{
		return privateMasks;
	}
	
	const std::vector<uint32_t>& PreprocessingBuilder::getUntruncatedMasks()
	{
		return unTruncatedMasks;
	}

	uint32_t PreprocessingBuilder::getMasks(int idx)
	{
		return privateMasks[idx];
	}
	
	void PreprocessingBuilder::setMasks(const std::vector<uint32_t>& value)
	{
		privateMasks = value;
	}
	
	void PreprocessingBuilder::setMasks(std::vector<uint32_t>&& value)
	{
		privateMasks = std::move(value);
	}

	void PreprocessingBuilder::setUntruncatedMasks(const std::vector<uint32_t>& m)
	{
		unTruncatedMasks = m;
	}
	
	std::vector<std::vector<uint32_t> > PreprocessingBuilder::getBobCorrection() 
	{
		return privateBobCorrection;
	}

	void PreprocessingBuilder::addBobCorrection(std::vector<uint32_t> value)
	{
		privateBobCorrection.push_back(value);
	}
	
	void PreprocessingBuilder::setBobCorrection(const std::vector<std::vector<uint32_t> >& value)
	{
...
// 		std::vector<int> maskIndex = CryptoUtility::buildMaskIndex(itemsPerUser);
		...
		setBobCorrection(std::vector<std::vector<uint32_t> >());

		std::cout << "Reconstruct incorrect private masks" << std::endl;
		std::vector<uint32_t> privateMask(numMasks);
...
// 		TestUtility::PrintByteArray(hashBeaver, "hash beaver");
		...
		std::vector<uint32_t> bobCorrections;
		int count = 0;
