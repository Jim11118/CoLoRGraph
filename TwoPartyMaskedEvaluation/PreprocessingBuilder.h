...
namespace TwoPartyMaskedEvaluation
{
	...
	public:
		...
		virtual const std::vector<uint32_t>& getMasks() = 0;

		virtual std::vector<std::vector<uint32_t> > getBobCorrection() = 0;
	};
	...
	private:
		...
		std::vector<uint32_t> privateMasks;
		std::vector<uint32_t> unTruncatedMasks;
		std::vector<std::vector<uint32_t> > privateBobCorrection;
	...
	public:
		...
		const std::vector<uint32_t>& getMasks();
		const std::vector<uint32_t>& getUntruncatedMasks();
		
		uint32_t getMasks(int idx);
		
		void setMasks(const std::vector<uint32_t>& value);
		void setMasks(std::vector<uint32_t>&& value);
		void setUntruncatedMasks(const std::vector<uint32_t>& m);
		
		std::vector<std::vector<uint32_t> > getBobCorrection();
		
		void addBobCorrection(std::vector<uint32_t> value);
		
		void setBobCorrection(const std::vector<std::vector<uint32_t> >& value);
