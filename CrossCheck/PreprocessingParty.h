...
namespace CrossCheck
{
	...
	public:
		...
		void RunPreprocessing(LayeredArithmeticCircuit *lc, std::vector<uint32_t>& masks, PreprocessingShare * &share, std::vector<uint32_t>& unTruncatedMasks, const std::vector<int>& itemsPerUser);
		...
		void BuildPreprocessing(std::vector<uint32_t>& masks, std::vector<uint32_t>& unTruncatedMasks, LayeredArithmeticCircuit *lc, const std::vector<int>& itemsPerUser, const std::vector<int>& shareIndex);
