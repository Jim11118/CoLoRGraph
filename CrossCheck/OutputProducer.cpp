...
namespace CrossCheck
{
	...
	OutputProducer::OutputProducer(Communicator *communicator, std::vector<std::vector<uint32_t> > &maskedEvaluation, std::vector<uint32_t> &mask)
	{
	...
	std::vector<std::vector<uint32_t> > OutputProducer::ComputeOutput(Range *range)
	{
		assert(range != nullptr);
		std::vector<uint32_t> outputsMasks(range->Length);
...
// // 		}
// 		
		std::vector<uint32_t> masks = ArrayEncoder::Decodeuint32_t(v1);
