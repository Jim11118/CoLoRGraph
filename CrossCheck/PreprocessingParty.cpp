...
namespace CrossCheck
{
	...
	void PreprocessingParty::RunPreprocessing(LayeredArithmeticCircuit *lc, std::vector<uint32_t>& masks, PreprocessingShare * &share, std::vector<uint32_t>& unTruncatedMasks, const std::vector<int>& itemsPerUser)
	{
...
// 		std::future<std::vector<uint32_t> > f1 = std::async(std::launch::async, [&]{return BuildPreprocessing(lc, itemsPerUser, shareIndex);} );
...
// 		share = f2.get();
	...
	void PreprocessingParty::BuildPreprocessing(std::vector<uint32_t>& masks, std::vector<uint32_t>& unTruncatedMasks, LayeredArithmeticCircuit *lc, const std::vector<int>& itemsPerUser, const std::vector<int>& shareIndex)
	{
		...
		std::vector<unsigned char> maskCorrection = ArrayEncoder::EncodeUInt32Array(builder->getBobCorrection());
...
// 			std::cout << "Alice sending bob mask correction: " << maskCorrection.size() << std::endl;
			uint32_t size = maskCorrection.size();
			communicator->SendBob((unsigned char *)(&size), sizeof(uint32_t));
...
// 			std::cout << "Else of AlicePreprocessingBuilder::Build(lc, mySeed1.data());" << std::endl;
			uint32_t size;
			communicator->AwaitAlice((unsigned char *)(&size), sizeof(uint32_t));
			...
			communicator->AwaitBob((unsigned char *)(&size), sizeof(uint32_t));
			...
			std::vector<std::vector<uint32_t> > maskCorrection = ArrayEncoder::DecodeUInt32Array(maskCorrectionEncoding);
			...
			for (int layer = 0;layer < lc->Depth;layer++)
			{
				...
				if (cMulGates.size() > 0)
				{
					std::vector<uint32_t> correction = maskCorrection[count]; count++;
					int count2 = 0;
