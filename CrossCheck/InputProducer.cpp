...
namespace CrossCheck
{
	...
	InputProducer::InputProducer(Communicator *communicator, std::vector<uint32_t> masks, MaskedEvaluation *eval, PreprocessingShare *share) : eval(eval), masks(masks), communicator(communicator), share(share)
	{
// 		MyDebug::NonNull(communicator, masks, eval,share);
	}

	void InputProducer::AddInput(IRange *myRange, IRange *evaluationPartnerRange, IRange *aliceRange, IRange *bobRange, IMaybe<std::vector<std::vector<uint32_t> > > *myInput)
	{
// 		MyDebug::NonNull(myRange, evaluationPartnerRange, aliceRange, bobRange, myInput);
		assert((dynamic_cast<Just<std::vector<std::vector<uint32_t> > *>(myInput) != nullptr) || (dynamic_cast<EmptyRange *>(myRange) != nullptr));
		ss << "AddInput\n";
	...
	std::vector<uint32_t> InputProducer::GetMasks(Range *range)
	{
// 		MyDebug::NonNull(range);
		std::vector<uint32_t> ret(range->Length);
	...
	void InputProducer::AddMyInput(IRange *myInputRange, IMaybe<std::vector<std::vector<uint32_t> > > *myInput)
	{
		if (dynamic_cast<Range*>(myInputRange) != nullptr && dynamic_cast<Just<std::vector<std::vector<uint32_t> > *>(myInput) != nullptr)
		{
			ss << "AddMyInput\n";
			Range *range = (Range *)myInputRange;
			std::vector<unsigned char> temp = communicator->AwaitEvaluationPartner();
			auto partnerMaskShare = ArrayEncoder::Decodeuint32_t(temp);

			std::vector<std::vector<uint32_t> > myMaskedInput(range->Length);
			std::vector<uint32_t> myShare = share->operator[](range);
...
// 		std::cout << ss.str() << std::endl;
	...
	void InputProducer::AddEvaluationInput(IRange *myInputRange, IRange *partnerInputRange, IMaybe<std::vector<std::vector<uint32_t> > > *myInput)
	{
// 		MyDebug::NonNull(myInputRange, partnerInputRange, myInput);
// 		assert(dynamic_cast<Just<std::vector<uint32_t> >*>(myInput) != nullptr ^ dynamic_cast<EmptyRange*>(myInputRange) != nullptr);
		...
		if (dynamic_cast<Range*>(partnerInputRange) != nullptr)
		{
			...
			std::vector<uint32_t> mask = share->operator[](range);
			...
			auto hisMaskedInput = ArrayEncoder::Decodeuint32_t(temp2);
	...
	void InputProducer::TransferInput(IRange *myInputRange, IRange *aliceRange, IRange *bobRange, IMaybe<std::vector<std::vector<uint32_t> > > *myInput)
	{
// 		MyDebug::NonNull(myInputRange, aliceRange, bobRange, myInput);
// 		assert(dynamic_cast<EmptyRange*>(myInputRange) != nullptr ^ dynamic_cast<Just<std::vector<uint32_t> >*>(myInput) != nullptr);
		ss << "TransferInput: ";
		if (dynamic_cast<Range*>(myInputRange) != nullptr)
		{
			ss << "dynamic_cast<Range*>(myInputRange) != nullptr" << std::endl;
			Range *range = (Range *)myInputRange;
			std::vector<std::vector<uint32_t> >  maskedInput(range->Length);
			for(int idx = 0; idx < range->Length; idx++)
			{
...
// 			std::cout << "dynamic_cast<Range*>(iRange) != nullptr" << std::endl;
			...
			eval->AddInput(ArrayEncoder::Decodeuint32_t(maskedInput), range);
			return maskedInput;
		}
		else
		{
